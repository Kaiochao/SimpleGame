Component/AimingHandler/player
	...
	proc
		Start()
			...
			EVENT_ADD(_input_handler.MouseMoved, src,
		...
		*/
		Update()
			...
			if(aim_input)
				...
				var Component/Camera/camera = GetComponent(
					/Component/Camera)
				if(camera)
					var
						camera_x = 0
		...
		GetAnalogAimInput()
			...
			var
				analog_aim[] = input_handler.GetAxisValues(aim_analog)
				aim_x = analog_aim[1]
				aim_y = analog_aim[2]
